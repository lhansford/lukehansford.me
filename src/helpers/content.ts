...
export type IBlogPost = IIndexData;
...
export function getContentItems<T>(contentType: ContentType): IContentItem<T>[] {
...
export function getContentItem<T>(contentType: ContentType, id: string): IContentItem<T> {
  return {
    directoryName: id,
...
export function getContentPaths(contentType: ContentType): { params: { id: string } }[] {
  return fs
    .readdirSync(`./public/contents/${contentType}`, { withFileTypes: true })
    .filter((result) => result.isDirectory())
